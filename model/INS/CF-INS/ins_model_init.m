...
INS_PARAM_VALUE.MAG_FLY_EN = uint8(0);
...
INS_PARAM_VALUE.GPS_POS_DELAY = uint32(100);    % in ms
INS_PARAM_VALUE.GPS_VEL_DELAY = uint32(100);    % in ms
INS_PARAM_VALUE.GPS_X_OFFSET = single(0);   % in m
INS_PARAM_VALUE.GPS_Y_OFFSET = single(0);   % in m
INS_PARAM_VALUE.GPS_Z_OFFSET = single(0);   % in m
INS_PARAM_VALUE.GPS_YAW_OFFSET = single(pi/2); % in rad
INS_PARAM_VALUE.GPS_YAW_EN = uint8(0);
INS_PARAM_VALUE.RTK_POS_GAIN = single(2.0);
INS_PARAM_VALUE.RTK_ALT_GAIN = single(2.0);
INS_PARAM_VALUE.RTK_VEL_GAIN = single(0);
INS_PARAM_VALUE.RTK_BIAS_A_GAIN = single(0);
INS_PARAM_VALUE.OPF_VEL_GAIN = single(2);
INS_PARAM_VALUE.OPF_BIAS_A_GAIN = single(1);
INS_PARAM_VALUE.OPF_VEL_DELAY = uint32(5);
INS_PARAM_VALUE.OPF_FIX_GAIN = single(0);
...
INS_PARAM_VALUE.RF_H_DELAY = uint32(5); % in ms
...
INS_PARAM_VALUE.EXTPOS_POS_DELAY = uint32(50);  % in ms
INS_PARAM_VALUE.EXTPOS_PSI_OFS = single(0); % in rad
